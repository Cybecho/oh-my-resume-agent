...
- `omr`: setup, status, paths, doctor, and release-gate checks. `resume` is an alias for the same CLI.
...
Not supported in v0: complete PDF/DOCX/HWP/PPTX/Excel/Notion ingestion, OCR for image-only job posts, guaranteed crawling for every recruiting site, direct LLM generation inside the `omr` CLI, and DOCX/Figma publish completion.
...
| Command | Purpose |
| --- | --- |
| `omr setup` | Text UI setup wizard for local folders, private source readiness, and job-post fallback readiness. |
| `omr init` | Create local user/workspace/state folders and placeholders. |
| `omr status` | Print private source counts, job-post fallback counts, workspace counts, and local state. |
| `omr doctor` | Check required folders/files and explain missing user data without pretending setup is complete. |
| `omr config` | Show what this CLI does and which KOR/EN agent pipelines to run. |
| `omr paths` | Print absolute paths for `userinfo`, `workspace`, `output`, and `.omr/state`. |
| `omr eval privacy` | Fail if tracked private paths or obvious PII patterns are found. |
| `omr eval skills` | Check Codex and Claude skill surfaces and document GitHub EN v0 unsupported status. |
...
Oh My Resume-Agent is not a standalone AI app that generates application documents from a single `omr` command. It is an agent harness that safely connects **local user data → normalized evidence → agent pipelines → output review**. The core v0 design separates responsibilities: the CLI handles preparation and validation, while Codex/Claude Code skills and agents handle the actual writing.
...
1. **Initialize**: `omr setup` or `omr init` creates local folders such as `userinfo/`, `workspace/`, `output/`, and `.omr/state/`.
...
4. **Validate readiness**: `omr doctor` checks required folders/files and whether user data exists. `omr eval privacy` checks that private data or generated real outputs are not tracked in the public repo, and `omr eval skills` checks the Codex/Claude skill surfaces.
...
- **CLI/writing separation**: the `omr` CLI only handles setup/status/doctor/eval. `resume` is an alias. Actual LLM-based writing runs in the Codex or Claude Code agent runtime.
