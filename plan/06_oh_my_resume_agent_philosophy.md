...
- `omr`/`resume` CLI는 사용자의 데이터 준비, 설정, 상태 확인, doctor, guide를 담당한다.
- KOR/EN 자기소개서 생성은 기존 skill/agent pipeline에서 수행한다.

따라서 `resume` 명령이 “AI 생성 버튼”이 되면 안 된다. `resume`은 사용자의 로컬 작업공간을 agent가 잘 사용할 수 있게 준비하는 control plane이다.
...
| --- | --- |
| `resume` CLI/TUI | `omr` control plane + `resume` alias |
